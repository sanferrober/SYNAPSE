...
# Script para desplegar Synapse con Docker

echo "=== Desplegando Synapse con Docker ==="
...
if [ $(docker-compose ps -q | wc -l) -eq 2 ]; then
    echo "=== Synapse se ha desplegado correctamente ==="
    echo "Backend: http://localhost:5000"
    echo "Frontend: http://localhost:3000"
    echo "Para detener los servicios, ejecute: docker-compose down"
else
    echo "Error: Hubo un problema al desplegar Synapse. Revise los logs con: docker-compose logs"
    exit 1
fi